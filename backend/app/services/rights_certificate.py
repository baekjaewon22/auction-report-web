...
from .selenium_driver import click_tab_safe, create_driver, login_myauction, safe_click, wait_document_ready
...
async def generate_rights_certificate(
    ...
    url = _normalize_detail_url(request.url, request.myauction_id)

    profile_dir = ""
    if request.remember_login:
        safe_id = re.sub(r"[^0-9A-Za-z._-]", "_", request.myauction_id)
        profile_dir = os.path.join(SELENIUM_PROFILE_DIR, f"rights_{safe_id}")
...
def _normalize_detail_url(raw_url: str, myauction_id: str) -> str:
    ...
    if "/view3/" in url:
        ...
        after_view3 = url[view3_idx + 7:].rstrip("/")
        parts = after_view3.split("/")
        if len(parts) == 1 and parts[0].isdigit():
            url = url.rstrip("/") + "/" + myauction_id
    return url
...
def analyze_surplus(data: dict, rights: list[dict], base_right: Optional[dict], related_cases: list[dict]) -> str:
    ...
    if _has_duplicate_auction_case(related_cases):
        ...
        no_surplus_text = "경매신청채권자는 순위배당시 배당을 받을 수 있으므로 무잉여 가능성은 없습니다."
        ...
        no_surplus_text = f"현재 경매 최저가는 {fmt_money(min_bid)}이므로 무잉여 가능성이 존재합니다."
